...
class CSVDataset(Dataset):
    def __init__(self, csv_folder, target_label=None):
        ...
        if target_label is not None:
            ...
            normalized_data = normalized_data * 2 - 1  # 归一化到[-1, 1]范围，与tanh输出匹配
...
class Generator(nn.Module):
    ...
    def forward(self, z, labels):
        ...
        shared_features = self.shared(gen_input)
        feature1 = self.branch1(shared_features)
        feature2 = self.branch2(shared_features)
...
def train_ctgan(csv_folder, target_label, n_epochs=500, batch_size=64, lr=0.0002, 
    ...
    for epoch in range(n_epochs):
        ...
        for i, (real_data, labels) in enumerate(dataloader):
            ...
            # 标签平滑
            real_label = torch.full((batch_size,), 0.9, dtype=torch.float).cuda()
            fake_label = torch.full((batch_size,), 0.1, dtype=torch.float).cuda()
            ...
            # 计算多个损失项
            g_loss_adv = criterion(fake_output, real_label)
...
if __name__ == "__main__":
    ...
    for label, fault_name in fault_types.items():
        ...
        save_dict = train_ctgan(
            csv_folder=csv_folder,
            target_label=label,
            n_epochs=50,      # 增加训练轮数
            batch_size=64,     # 较小的batch size
            lr=0.0002,
            latent_dim=100,
            beta1=0.5,
            beta2=0.999
        ...
        # 保存模型
        save_path = os.path.join(save_dir, f"ctgan_generator_{fault_name}.pth")
